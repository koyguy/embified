...
import type { ChatMessage, MediaKind, WaStatus } from '../src/types.ts';
...
let status: WaStatus = { connected: false, qr: null, me: null, error: null };
let sock: any = null;
...
async function handleIncoming(msg: any) {
  ...
  if (!isJidGroup(remoteJid)) return; // groups only
  ...
  store.upsertGroup({ id: groupId, name: groupName, memberCount });
...
export async function startWhatsApp() {
  ...
  sock.ev.on('connection.update', async (update: any) => {
    ...
    if (connection === 'open') {
      ...
      emit('status', status);
      logger.info({ me: status.me }, 'WhatsApp connected');
      await syncGroups();
    }
    if (connection === 'close') {
      ...
      status = {
        ...
        qr: null,
        me: null,
        error: loggedOut ? 'Logged out — delete auth_info and restart to rescan QR' : `Disconnected (${code || 'unknown'})`,
      };
      emit('status', status);
      if (loggedOut) {
  ...
  process.on('unhandledRejection', (err: any) => {
    logger.warn({ err: err?.message || err }, 'unhandledRejection');
  });
}
